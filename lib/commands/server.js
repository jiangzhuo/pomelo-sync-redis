...
exports.flush = function () {
    ...
    if (typeof cb != 'function') {
        ...
        switch (arguments.length) {
            case 4:
                mergerKey = [arguments[0], arguments[1]].join('')
                this.redis.hdel(this.MERGER_MAP_KEY, mergerKey, cb)
                this.rewriter.flush(arguments[0], arguments[2])
                break
            case 5:
                mergerKey = [arguments[0], arguments[1]].join('')
                var multi = this.redis.multi()
                ...
                multi.exec(cb)
                this.rewriter.flush(arguments[0], arguments[3])
                break
            default :
            ...
            if (res != null) {
                for (var i = 0; i < res.length; i++) {
                    ...
                    self.rewriter.tick(mergerMapValue, function (err, res) {
                        ...
                        console.log('flush all byy uid: begin delete mergerMap :')
                        console.log(res)
                        multi.hdel(self.MERGER_MAP_KEY, res.mergerKey)
                        console.log('flush all byy uid: begin delete userSet userid: ' + res.uid + " mergerKey :" + res.mergerKey)
                        if (res.uid != null) {
